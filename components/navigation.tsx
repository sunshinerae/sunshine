            ...
            <nav className="hidden lg:flex items-center gap-1">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="relative px-3 py-2 text-sm font-medium text-sunshine-brown hover:text-sunshine-purple transition-all duration-300 rounded-full hover:bg-sunshine-purple/5 group"
                >
                  <span className="relative">
                    {item.label}
                    <span className="absolute -bottom-0.5 left-0 w-0 h-0.5 bg-sunshine-purple rounded-full transition-all duration-300 group-hover:w-full" />
                  </span>
                </Link>
              ))}
            </nav>
            ...
            <nav className="flex flex-col gap-2">
              {navItems.map((item) => (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setMobileMenuOpen(false)}
                  className="relative px-4 py-3 text-lg font-medium text-sunshine-brown hover:text-sunshine-purple hover:bg-sunshine-purple/5 transition-all duration-300 rounded-xl group"
                >
                  <span className="relative inline-flex items-center gap-2">
                    <span className="w-0 h-0.5 bg-sunshine-purple rounded-full transition-all duration-300 group-hover:w-3" />
                    {item.label}
                  </span>
                </Link>
              ))}
